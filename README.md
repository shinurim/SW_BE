...
<ul>
  <li>
    <strong>Stage1 변환 (NL → SQL/Opinion/Main/Sub)</strong>:
  ...
  <li>
    <strong>Opinion 기반 고정밀 패널 추출</strong>:
  ...
  <li>
    <strong>Loyalty 기반 우선순위 제공 (Direct Filtering 전용)</strong>:
  ...
  <li>
    <strong>Checkbox 기반 Direct Filtering</strong>:
    체크박스 필터 또한 <strong>Direct Filtering</strong> 방식을 채택하여
    <strong>5초 내</strong> 적합한 패널을 빠르게 추출합니다.
  </li>

  <li>
    <strong>주관 질의 기반 인사이트 생성</strong>:
    주관이 포함된 자연어 질의의 경우 적합 그룹의 설문 응답을 바탕으로
    <strong>적절한 인사이트를 생성</strong>합니다.
  </li>

  <li>
    <strong>적합 그룹 vs 전체 그룹 비교 분석</strong>:
    적합 그룹의 설문 특징과 전체 그룹의 설문 특징을 <strong>비교 분석</strong>하여
    차별화된 인사이트를 제공합니다.
  </li>

  <li>
    <strong>질의 추천 (3개)</strong>:
    <ul>
      <li><strong>유사 질의 2개</strong> 추천</li>
      <li>
        <strong>비유사(반대 선상) 질의 1개</strong> 추천
        <span style="color:#666;">(예: 좋아하는 ↔ 싫어하는)</span> —
        적합 그룹의 특징을 더 명확히 파악할 수 있도록 지원합니다.
      </li>
    </ul>
  </li>

  <li>
    <strong>세그먼트 저장</strong>:
    생성된 인사이트를 <strong>세그먼트 형태로 저장</strong>하여 추후 재활용할 수 있습니다.
  </li>
</ul>
